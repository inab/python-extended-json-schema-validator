...
class AbstractCustomFeatureValidator(abc.ABC):
	...
	@abc.abstractmethod
	def validate(self, validator: "JSV.validators._Validator", schema_attr_val: "Any", value: "Any", schema: "Any") ->  "Iterator[ValidationError]":
		pass
	...
	def invalidateCaches(self) -> None:
		self.InvalidateCache(cachePath=cast("Optional[str]", self.config.get('cacheDir')))
	...
	def doSecondPass(self, l_customFeatureValidators: "Mapping[str, Sequence[CheckContext]]") -> "Tuple[Set[str], Set[str], Sequence[SecondPassErrorDict]]":
		return set() , set() , []
